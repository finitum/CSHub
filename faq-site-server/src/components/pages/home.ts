...
import {IEdit, IPost, ITopic} from "../../../../faq-site-shared/models";

app.get(GetHomeRequest.getURL, (req: Request, res: Response) => {
            ...
            const getChildTopics = (id: number): ITopic[] => {
                ...
                if (childTopics.length > 0) {
                    ...
                    for (const topic of childTopics) {
                        childTopicsParsed.push({
                            children: getChildTopics(DatabaseResultSet.getNumberFromDB("id", topic)),
                    ...
                    async.timesSeries(convertedPostsRows.length, (currentRun: number, next) => {
                        ...
                        `, post.getNumberFromDB("id"))
                            .then((edits: DatabaseResultSet) => {
                                ...
                                postsObj.push({
                                    edits: currEdits,
                                    topic: topicsParsed.find(x => x.id === post.getNumberFromDB("topicId")),
                                    author: {
                        ...
                        const callbackObj = new GetHomeCallBack(postsObj, topicsParsed);

...
export function registerCheckAnswersEndpoint(app: Application): void {
    app.post(CheckAnswers.getURL, (req: Request, res: Response) => {
        ...
        async function checkClosedQuestion(
            ...
            if (clientAnswer.type === question.type) {
                ...
                if (
                    parsedQuestion.type !== QuestionType.SINGLECLOSED &&
                    parsedQuestion.type !== QuestionType.MULTICLOSED
                ) {
        ...
        async function checkDynamicQuestion(
            clientAnswer: CheckAnswerType,
            question: Question,
        ): Promise<CheckedAnswerType> {

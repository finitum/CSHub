...
app.put(EditPost.getURL, (req: Request, res: Response) => {
    ...
    const inputsValidation = validateMultipleInputs({input: postHash}, {input: editPostRequest.postTitle}, {input: editPostRequest.postTopicHash}, {input: editPostRequest.deleteEdit});

    if (inputsValidation.valid && userObj.valid) {

        const userPostAccessPromise = hasAccessToPostRequest(postHash, req);

        userPostAccessPromise
            .then(value => {
                if (value.canSave) {

...
export const customValidator = (input: ICustomValidatorInput): ICustomValidatorResponse => {
    if (typeof input.input !== "undefined") {
        ...
        if (
            input.validationObject.nullemptyundefined &&
            (input.input === null || input.input === undefined || input.input === "")
        ) {
            ...
            input.validationObject.minlength &&
            input.validationObject.minlength > 0 &&
            inputString.length < input.validationObject.minlength
        ) {
            return { valid: false, error: CustomValidatorReponseTypes.MINLENGTH, value: input.input };
        } else if (
            input.validationObject.maxlength &&
            input.validationObject.maxlength > 0 &&

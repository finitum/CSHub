<template>
    <div>
            ...
            <v-card :class="{previewCard: !fullPostComputed, fullCard: fullPostComputed}" :id="'post_' + domId">
                    <v-card-title primary-title :id="'postTitle_' + domId" class="pb-0 xl-0 pt-1"  @click.capture="navigateToPost">
                        ...
                        <span v-if="showTopMenu || !fullPostComputed" style="display: contents">
                            ...
                            <v-list dense three-line class="pa-0">
                              <v-list-tile avatar>
                                <v-list-tile-avatar >
                                  <img :src="post.author.avatar" :class="{adminBorder: post.author.admin}"/>
                                </v-list-tile-avatar>
                                <v-list-tile-content class="pt-2 d-inline">
                                  <v-list-tile-sub-title class="black--text post-title">{{post.title}}</v-list-tile-sub-title>
                                  <v-list-tile-sub-title>{{post.author.firstname}} {{post.author.lastname}} - {{post.datetime | formatDate}}</v-list-tile-sub-title>
                                </v-list-tile-content>
                              </v-list-tile>
                            </v-list>
                        </span>
        ...
        <PostEditsDialog :postHash="postHash" />
    </div>
</template>
